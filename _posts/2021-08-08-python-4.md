...
이제 데이터를 정상적으로 추출했으니, 이 정보들을 저장하는 코드를 작성했다. 저장 방식은 txt파일과 xlsx파일, 이렇게 두 가지 방식을 이용하였다.
...
이 과정을 통해 추출한 데이터들을 txt파일과 xlsx파일, 2가지 형태로 저장하였다. 저장 결과는 다음과 같다.
...
　
 ...
 　
  ...
  책으로 Web Scraping 챕터는 끝났다. 이제 다음 챕터는 웹 API이다. 여기는 아예 해본 적이 없는 부분이라서 꼼꼼하게 공부를 해봐야겠다. 챕터 분량도 많은 것 같고, 얼핏 스윽 봤는데 꽤 흥미로운 듯.
